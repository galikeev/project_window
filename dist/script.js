...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('#timer', '2021-08-20');
...
var images = function images() {
  ...
  workSection.addEventListener('click', function (e) {
    ...
    if (target && target.classList.contains('preview')) {
      /* Есть ли событие клик и есть класс превью */
      imgPopup.style.display = 'flex';
      document.body.style.overflow = 'hidden';
      var path = target.parentNode.getAttribute('href');
      /* У элемента на котором произошло событие(img) обращаемся к его родителю и получаем нужный атрибут */

      bigImage.setAttribute('src', path);
      /* К изображению добавляем ссылку */
...
var modals = function modals() {
  var scrollHide = Object(_calcScroll__WEBPACK_IMPORTED_MODULE_1__["default"])();
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', function (e) {
    var target = e.target;
    /* Тот элемент на котором произошло событие, куда кликнул пользователь */

    if (target && (target.classList.contains(tabSelector.replace(/\./, '')) || target.parentNode.classList.contains(tabSelector.replace(/\./, '')))) {
      /* Проверяем, что действительно кликнули в один из табов */
